...
const FolderIcon = (elem, horizontalSize, kind, selectFolder, directories,readFileContent) => {
      ...
      > 
        <div>
          <svg xmlns="http://www.w3.org/2000/svg" 
              ...
              viewBox="0 0 512 512"
            >
            {kind==='revertButton'&&<path d="M 55 410 H 350 C 535 412 522 162 350 160 H 85 L 120 125 C 135 110 115 90 100 105 L 65 140 C 45 160 45 185 65 205 L 100 245 C 115 260 135 240 120 225 L 85 185 H 350 C 485 193 507 379 350 385 H 54.914 C 35 385 35 410 55 410"/>}
...
function GetListFolderAndFiles(size, folders, files, selectFolder, directories,readFileContent){
  // console.log('folders->>',folders)
  // console.log('files->>',files)
  // console.log('directories->>',directories)
  // console.log('directories.length->>',directories?.length)
  console.log('directories.flag ',(directories?.length !== 1 && !!directories?.length))
  ...
  let vLine ;
  let vListTMP = []
   if( !( directories?.length === 1) && directories) { /*создаем или нет первую кнопку "revert" для не первой папки*/
    vListTMP.push(FolderIcon(null,horizontalSize, 'revertButton', selectFolder,directories))
    lineInCounter++;
   }
  if(lineInCounter===horizontalSize){ /*если требуется создаем линию*/
    vLine = <div 
  ...
  ListFolderAndLists.forEach((listElem,index)=>{  /*по списку файлов и папок создаем значки, объединеяя в линии*/
    vListTMP.push(FolderIcon(listElem,horizontalSize, listElem.kind, selectFolder,null,readFileContent))
    lineInCounter++;
  ...
  if(vListTMP) { /*нужно для случая когда в папке нет ничего, потому как линия еще не добавилась*/
    vLine = <div 
              style={{display:"inline-block",  width:'100%'}} 
            >
              {vListTMP.map((elem)=>elem)}
...
const FolderListWrapper = (props) => {
  ...
  return (
    <div className="flex-1 p-1" style ={{borderColor:"#f0ffff",backgroundColor:"#f0ffff", borderStyle:"groove", width:'100%', height: "100%"}}>
      {GetListFolderAndFiles(size, foldersHandler, filesHandler, props.selectFolder, props.directories, props.readFileContent)}
    </div>
  )
}
